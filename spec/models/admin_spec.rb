...
RSpec.describe Admin, type: :model do
  describe '#create' do
    ...
    it 'お名前（全角）の苗字（last_name）が漢字またはひらがなまたはカタカナ以外では登録されないこと' do
      ...
      @admin.valid?
      expect(@admin.errors.full_messages).to include('Last name は全角（漢字・ひらがな・カタカナ）を使用してください')
    end
    ...
    it 'お名前（全角）の名前（first_name）が漢字またはひらがなまたはカタカナ以外では登録されないこと' do
      ...
      @admin.valid?
      expect(@admin.errors.full_messages).to include('First name は全角（漢字・ひらがな・カタカナ）を使用してください')
    end
    ...
    it 'お名前カナ（全角）の苗字（last_name_kana）がカタカナ以外では登録されないこと' do
      ...
      @admin.valid?
      expect(@admin.errors.full_messages).to include('Last name kana は全角（カタカナ）を使用してください')
    end
    ...
    it 'お名前カナ（全角）の名前（first_name_kana）がカタカナ以外では登録されないこと' do
      ...
      @admin.valid?
      expect(@admin.errors.full_messages).to include('First name kana は全角（カタカナ）を使用してください')
    end

    it '電話番号を入力する必要があること' do
      @admin.phone_number = nil
      @admin.valid?
      expect(@admin.errors.full_messages).to include("Phone number は「数字のみ且つ11桁以内で入力してください")
    end
    ...
    it 'passwordが数字だけでは登録されないこと' do
      ...
      @admin.valid?
      expect(@admin.errors.full_messages).to include('Password は英字と数字の両方を含めて設定してください')
    end
