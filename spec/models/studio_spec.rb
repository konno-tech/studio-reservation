...
RSpec.describe Studio, type: :model do
  ...
  describe '#create' do
    ...
    it 'お名前（全角）の苗字（last_name）が空では登録されないこと' do
      @studio.name = nil
      @studio.valid?
      expect(@studio.errors.full_messages).to include("Name can't be blank")
    end

    it '価格を入力しなければ登録できないこと' do
      @studio.price = nil
      @studio.valid?
      expect(@studio.errors.full_messages).to include("Price can't be blank")
    end

    it '価格が、¥500未満の場合登録できないこと' do
      @studio.price = 499
      @studio.valid?
      expect(@studio.errors.full_messages).to include('Price is not included in the list')
    end
    ...
    it '価格は半角数字以外では登録できないこと' do
      @studio.price = '１０００'
      @studio.valid?
      expect(@studio.errors.full_messages).to include('Price Half-width number')
    end
